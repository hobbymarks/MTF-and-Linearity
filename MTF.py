...
class MTF:
    def __init__(
        ...
        center = [
            center_offset[0] + img.shape[0] // 2,
            center_offset[1] + img.shape[1] // 2,
        ]
        if raw:
            center[0] += 10
            center[1] += 10
    ...
    def mtf(self, wave, freq, filt=True, trim=10):
        ...
        m = (yhat[maximums].mean() - yhat[minimums].mean()) / (
            yhat[maximums].mean() + yhat[minimums].mean()
        )
        return m
